...
| **Atualizado** | **08/08/2026 — A FOLHA DO PORTAL VOLTOU PREENCHIDA, E ELA MUDA TRES COISAS.** Leitura completa no bloco de 08/08 do `PLANO-leitura-fatura-equatorial-2026-08-07.md`. **(1) O PDF TEM CAMADA DE TEXTO** — o extrator vira **deterministico e de custo zero por documento**, e o modelo de visao cai para caminho de excecao. Login e **UC + CPF sem conta cadastrada**, e ha **156 meses** de historico com competencia passada. **(2) O E-MAIL PODE APAGAR A RASPAGEM INTEIRA:** a Equatorial oferece fatura por e-mail **apontavel para um e-mail nosso** — sem protecao de bot para contornar (o portal responde **403 Imperva**), sem quebrar quando o `.aspx` mudar, e e recurso OFERECIDO em vez de uso que os termos podem proibir. `Q-EQTL-CANAL-01` 🔴, e as duas portas ja construidas acomodam os dois canais **sem mudar uma linha**. **(3) APARECEU UM INSUMO HUMANO QUE NAO ESTAVA EM LISTA NENHUMA:** a validacao do login e a **data de nascimento** do titular — nao esta no schema, nao vem do CRM, nao e o item 1.1, e **para CNPJ ninguem sabe qual e**. E a quinta vez que um insumo aparece percorrendo o caminho em vez de auditando spec (`Q-EQTL-NASCIMENTO-01` 🔴). **A FATURA VEIO E FOI LIDA — e ela NAO TEM GERACAO DISTRIBUIDA.** `B1 RESIDENCIAL CONVENCIONAL`, duas linhas cobraveis somando o total exato, **sem energia compensada e sem nenhuma linha de fio B**; a folha marcava "tem compensacao" e, medido, nao tem. Ficou sabido que a iluminacao publica e `CONTRIB. ILUM. PUBLICA - MUNICIPAL`, que **nao existe linha chamada "encargos"** (encargos setoriais vao embutidos na tarifa) e que os **tributos vem destacados mas EMBUTIDOS** — soma-los cobra tributo duas vezes. **Falta uma coisa so: a fatura de uma das 29 UCs que faturam**, todas com rateio ativado. **DOIS CONSERTOS EM MIM MESMO:** `competenciaDe` aceitava `MM/AAAA` e `AAAA-MM`, e a fatura real escreve **`FEV/2026`** — o modulo teria **recusado todas**, com erro honesto, e ninguem saberia por que ate abrir um PDF (`F7c`/`F7d`); e o **`sha256` NAO e identidade estavel** como eu escrevi — o PDF carrega data de emissao e protocolo, entao reemitir gera bytes novos para a mesma fatura, e a ancora e a **chave NF3e**. **1660 → 1662 verificacoes**, `EXIT=0`, zero migrations · **07/08/2026, sessao 24 — A FRENTE NOVA: LEITURA AUTOMATICA DA FATURA DA EQUATORIAL.** Pedido do dono, com o Sicoob adiado de proposito para a ultima etapa. **Plano em `PLANO-leitura-fatura-equatorial-2026-08-07.md`, folha de campo em `EQUATORIAL-portal-2026-08-07.md`.** **(1) A fila foi revisada e nao mudou** — os sete itens da `RETOMADA-2026-08-06` §3 continuam abertos e continuam sem uma linha de codigo. **O que mudou e o peso do item 1.1:** medido em 07/08, o acesso a agencia virtual da Equatorial e **numero da UC + CPF/CNPJ do titular** — o mesmo insumo que ja era o primeiro da fila e que esta em **0 de 29**. Ele deixou de destravar uma coisa e passa a destravar **duas filas**. **(2) O achado que decide o desenho: A COLUNA E ESTREITA.** O `PRD` §5.1 define `valor_tarifas_concessionaria` como **fio B + iluminacao publica + encargos** — repasse puro, sobre o qual ninguem comissiona nem repassa. Uma fatura da Equatorial tem **tres numeros grandes na mesma pagina** e os tres parecem certos; ler o **total a pagar** no lugar da soma das tres parcelas **nao gera excecao, nao gera log e nao gera recusa** — gera fatura maior, repasse sobre base errada (§5.2), comissao errada (§5.4) e `liquido_g3` errado (§5.5), porque a formula SUBTRAI justamente essa parcela. Por isso a soma e explicita por componente, o total entra como **conferencia e nunca como fonte**, e a verificacao `F2c` afirma que os dois sao **diferentes de proposito**. **(3) Construido o que nao depende de ninguem:** `src/concessionaria/porta.ts` (duas portas — coletar e ler falham por motivos que nao tem nada em comum), `src/concessionaria/falso.ts` (portal e extrator falsos, deterministas, relogio injetado) e `src/dominio/fatura-concessionaria.ts` (puro). **Regra 5 na forma do tipo:** nao ha usuario, senha nem cookie em tipo nenhum — circula `credencial_ref`, e um tipo que aceitasse o segredo faria a violacao COMPILAR. **1618 → 1660 verificacoes**, `EXIT=0`, **zero migrations**, nada escrito em producao. **(4) O que NAO foi feito, e de proposito:** o coletor em Playwright (o portal responde **403 com Imperva/Incapsula** a quem nao e navegador — medido nos tres hosts — e escrever scraper contra portal que ninguem navegou e a `Q-PECA-NAO-PLUGADA-01` em forma pura), a migration 26 e a escolha do escopo das UCs. **Seis questoes novas no `QUESTOES.md`, quatro vermelhas com prazo** — `Q-EQTL-ESCOPO-01` (*"ativas"* sao **41** ou **29**? as duas leituras sao defensaveis), `Q-EQTL-CAMPOS-01` (qual rotulo da fatura e qual campo — falta uma fatura REAL, e metade e do contador), `Q-EQTL-CRED-01` (o par UC+CPF e dado de negocio E credencial de terceiro ao mesmo tempo) e `Q-EQTL-AUTORIZACAO-01` (mandato, termos de uso e LGPD). **Um erro meu registrado:** a verificacao `F4c` mandava `UC 132.900-60` e esperava que passasse — aceitar o rotulo exigiria decidir que ALGUMAS letras sao rotulo e outras sao zero mal lido, que e a adivinhacao que `F4d` proibe. **O teste estava errado, nao o codigo** · **06/08/2026, sessao 23 — MITIGAR TUDO QUE DA, ANTES DO PORTAL DA SICOOB.** Nenhuma decisao tomada e nenhuma migration: o que a sessao fez foi tirar do caminho o que nao precisa de ninguem de fora. **(1) A aba Documento deixou de ser suposicao.** A `RETOMADA` dizia que nenhum comando prova que ela abre com a chave listada pelo apelido — o bundle SERVIDO por producao (`sha256` conferido contra `web/dist`) foi renderizado em Chromium com a linha real de `chave_pix`, e o seletor sai com **`G3 GESTAO ENERGIA SOLAR — 66714022000121`**. O que isso NAO cobre continua dito: a camada HTTP com credencial valida so o login do dono fecha. **(2) A `Q-TARIFA-CONC-01` perdeu o silencio.** Entre compor e emitir ha um terceiro passo — `npm run tarifas` — que nenhuma lista tinha; sem ele a fatura sai cobrando so o credito, e `valor_total_centavos` e coluna GERADA, entao a parcela ausente vale zero e nada falha. A tela de Faturas e o `npm run faturar` agora **contam** e nomeiam as UCs, e **nao travam nada**: zero pode ser o valor certo, e essa e a pergunta (a), que tem dono (`B10f` prende isso). **(3) O quarto caso da `Q-PECA-NAO-PLUGADA-01`, e este estava no caminho critico:** a `RETOMADA` §2 dizia que alinhar o nome do recebedor ao do DICT *"custa um comando"* — e **o comando nao existia**. `PUT /cobranca/chaves-pix/:id` tinha ZERO chamadores, a aba Documento so cadastra, e `chave_pix_chave_unica` impede recadastrar. Plugado em `npm run identidade -- --editar`, com ensaio e diff campo a campo; `editarChavePix` tambem nao tinha teste e ganhou quatro, entre eles o sentido perigoso — editar sem devolver `titular_*` **apaga os tres**. **1607 → 1618 verificacoes**, `EXIT=0`, catalogo 9/9 contra producao, **zero migrations**. **PUBLICADO as 21:39 do mesmo dia** — segunda leva de 06/08 e a unica que TOCA A SPA: bundle `index-DN-7K2b1.js`, `sha256` do servido identico ao compilado, e **o aviso da tarifa conferido DENTRO do bundle servido** e nao pelo nome do arquivo. Guarda de arranque com 37 tabelas, catalogo 9/9 depois do restart, dados intactos (86 clientes, 41 UCs, 0 contratos, 0 faturas) e a aba Documento **renderizada de novo sobre o bundle novo**. Sem janela de bundle-novo-com-API-velha desta vez: nenhuma rota mudou · **sessao 22 — a ULTIMA MILHA DA FATURA.** O documento passou a sair **em lote** (`web/src/lote-de-documentos.ts`, modo *"imprimir o mes inteiro"* na aba Documento), e com isso **nenhum passo do caminho da primeira fatura e mais codigo** — o que falta e insumo humano ou decisao com dono. **1569 verificacoes**, `EXIT=0`, **zero migrations**. **A PALETA TROCOU INTEIRA no fim do dia** — Navy `#14213D` + Creme `#F6F2EA`, com o laranja passando de identidade a **acao**: a marca agora e o Navy, o topo virou faixa escura e o laranja ficou reservado ao que se clica e ao que se paga. Tres das cores entregues **nao passam AA onde foram pedidas** (Gray 2.75:1, Orange 2.41:1, Gold 1.80:1 sobre o creme) e ganharam token derivado na mesma matiz, achado por busca. O defeito que so a FOTO pegou: no primeiro tema escuro a faixa usou a variante `#1C2C4E`, que e o valor do cartao — contraste de texto perfeito, e barra e cartao indistinguiveis na tela. A `Q-PAGADOR-01` cai de 🔴 para 🟡: `boleto.registrar()` recusa pagador sem CPF/CNPJ com erro **nomeado e 422** — antes de tocar a porta e **antes de criar a linha**, que poria na fila do `PRD` §6 um boleto que nao pode dar certo, e a fila nunca desiste sozinha. Dois defeitos de impressao medidos **por PDF em Chromium**, e o maior e anterior: imprimir *uma* fatura produzia **cinco** paginas, quatro em branco (`visibility: hidden` esconde e **nao desocupa**). Duas coisas que o caminho ponta a ponta achou e que **nao estavam em lista nenhuma**: sem `npm run tarifas` **entre** compor e emitir a fatura sai cobrando so o credito, em silencio (`Q-TARIFA-CONC-01` reaberta 🟡); e pela `Q-FATCHEIA-01` a competencia 2026-06 sairia com **0 de 29 faturas cheias** — comissao nenhuma na liquidacao. **PUBLICADO no mesmo dia, em duas levas** — a fatura em lote as 13h55 e a paleta as 14h44, zero migrations nas duas. O bundle `index-Behx_OZi.js` e byte-identico ao compilado no Codespace, e a paleta foi conferida DENTRO do bundle servido e nao so pelo nome do arquivo · sessao 20: **duas entregas.** (1) **O LAYOUT DA FATURA VIROU POSICAO**, a pedido do dono: escolher o papel (A4, A5, Carta, Oficio), a orientacao, as margens, e **arrastar cada elemento para onde se quer** — migration **23**, `src/dominio/layout-visual.ts` puro e o editor na aba Documento. Tres defeitos silenciosos consertados junto: o `@page` **nao declarava `size`** (o papel era o do sistema de quem imprime), a previa tinha geometria diferente da do papel, e o negrito do total **nao viajava no payload** — o CRM receberia a fatura sem ele. Plano e execucao em `PLANO-layout-visual-2026-08-03.md`. (2) **`Q-VIEWSCRED-01` FECHADA: o conector le as DEZ views e confere o eixo do originador** — `SPEC-002` R26 e invariante 14, **1262 verificacoes** (`EXIT=0`, delta **33** no conector, **74** no layout e **7** na situacao do rateio contado na fonte e conferido contra o `npm test`, diferenca zero). Medido antes de construir: **39 das 39 UCs espelhadas tem credito vigente**, e o conector **nao escreve originador** — divergencia vira sinal, com as cinco condicoes mudas contra o estado de hoje. A causa raiz foi consertada junto: nada comparava as views do CRM contra a nossa lista fechada, e agora `viewsNovasNoCrm`/`viewsAusentes` gritam as duas direcoes. Nova 🟡 `Q-SITUACAO-01`: **11 das 39 UCs estao `nao_ativado` no CRM**, 7 em troca de titularidade · sessao 19: **`RESUMO-SESSAO-19.md`** · **1123 verificacoes**, delta de **17** · **o dev do CRM respondeu, e o eixo do originador nao era nenhuma das duas colunas que mediamos**: e o **credito congelado no momento do ganho**. Consequencia medida: o mapa de 30/07 esta errado em **12 das 41 UCs**, e **19,6% da carteira em kWh** troca de dono · **tres afirmacoes do dev estavam vencidas**, inclusive a de que nao alcancavamos o dado — as duas views existem e sao legiveis · nova 🔴 `Q-PARCERIA-01`: o credito traz **vendedor E parceiro** e o `contrato` guarda um so · migration 22 (`contas_a_pagar`) **so em banco de teste** · nada escrito em producao |
| **No ar** | **`https://financeiro.blackhaus.io`** — systemd, Node 22 isolado, TLS ate 26/10. **14/08/2026 15:48 — A ETAPA DE CADASTRO DA FATURA SAI DA BARRA, por decisao do dono (*"deixe a etapa de cadastro da fatura oculta por enquanto"*).** `origin/main` em **`bf46eda`**, **31 migrations = 31** (nenhuma). Bundle **`index-PiqfrfV6.js` -> `index-S70UaBn3.js`** e a aba **`documento-DeJVQMxJ.js` -> `documento-CKP0tNEr.js`**, os dois **byte-identicos** ao compilado no Codespace (`sha256`), e os dois pedacos da leva anterior respondem **404**. Guarda de arranque: *"client gerado cobre as 37 tabelas de public"*, **0 religamentos**. **O sinal desta leva esta dentro do pedaco servido:** ele carrega `["leitura","emissao"]` — a ordem de DUAS abas — e nao existe array de tres; e carrega o ouvinte de `hashchange`, que e a porta. **OCULTA E NAO REMOVIDA, e a distincao veio de medicao feita antes:** producao tem os **cinco campos do emissor vazios e nenhuma logo** (`razao_social`, `cnpj`, `telefone`, `endereco`, `email`), e esta aba e o unico caminho de tela para eles, para a chave Pix, o modelo e os campos personalizados. Enquanto estiverem vazios a folha sai **sem a linha do emissor e sem o Beneficiario** do boleto — o nome ao qual o aviso anti-golpe se amarra. Ela continua em **`/documento#cadastro`**, e o fragmento vale nos dois sentidos sem recarregar. A ordem das abas virou modulo puro (`web/src/abas-da-fatura.ts`) para poder ser verificada — **7 verificacoes novas** (`I9`..`I9g`), e a que mais importa e a `I9e`: apagar o `#` estando DENTRO do cadastro recua para a primeira aba, senao a barra fica com nenhuma selecionada e **nenhuma no caminho do `Tab`**. `npm test` **EXIT=0, 1911 verificacoes**. **14/08/2026 15:07 — A ABA DOCUMENTO VIRA A REFERENCIA, E E DEPLOY SO DE INTERFACE.** `origin/main` em **`c283d8a`**, **31 migrations = 31** (nenhuma nesta leva — conferido pela `DIRECT_URL` **antes** de subir, e por isso o `migrate deploy` foi pulado em vez de rodado a esmo). Bundle **`index-CyRHXU7x.js` -> `index-PiqfrfV6.js`** e o pedaco da aba **`documento-B1fvTxPi.js` -> `documento-DeJVQMxJ.js`**, os dois **byte-identicos** ao compilado no Codespace (`sha256` conferido nos dois). Guarda de arranque: *"client gerado cobre as 37 tabelas de public"* — inalterado, porque o schema nao mudou. **O sinal que prova esta leva nao e rota nenhuma: sao as FONTES.** `GET /fontes/barlow-600-latin-v1.408.woff2` respondia **404** antes e responde **200** agora; as **16** foram conferidas uma a uma no ar — codigo 200, `content-type: font/woff2`, `cache-control: immutable` e **`sha256` igual ao do arquivo local nas 16**. Nem rota nem schema mudaram nesta leva, entao 401 nao provaria nada e 404 de rota removida nao existia para provar. **E a conferencia foi alem do arquivo:** carregada a pagina real em navegador, o CSS servido traz **103 regras `.g3ref`** e **16 `@font-face` da Barlow**, com **zero erro de console ou de rede**, e os tokens da ilha resolvem **nos dois temas** (claro `--g3ref-apagado: #8F939D`; escuro `#999DA8`) — o modo de falha silencioso aqui seria token vazio, e nenhum veio vazio. Catalogo **9/9 contra producao** e dados intactos: **88 clientes, 41 UCs, 0 contratos, 0 faturas, 0 registros unificados** (eram 86 clientes no registro de 12:58 — os 2 sao do CRM, nao deste deploy). **Passo 0 rodado** (`npm install` na raiz): `up to date`, nenhuma dependencia nova nesta leva. Pendencias que continuam sendo do dono e nao de deploy: a `ANTHROPIC_API_KEY` fora do `/etc/financeiro.env`, o emissor vazio e as 41 UCs com `tarifa_reais_por_kwh` NULL. **14/08/2026 12:58 — A ABA DOCUMENTO PERDE O PROCESSO ANTERIOR.** `origin/main` em `55afc25`, **31 migrations = 31** (nenhuma nesta leva — a mudanca e so de interface). Bundle `index-CyRHXU7x.js` e o pedaco da aba `documento-B1fvTxPi.js` (**64,40 -> 51,65 KB**), os dois **byte-identicos ao compilado no Codespace**. Guarda de arranque: *"client gerado cobre as 37 tabelas de public"*. **O sinal que prova o codigo novo: o pedaco ANTIGO da aba, `documento-qp6_YP6t.js`, responde 404** — 401 nas rotas nao provaria nada, e aqui nem rota mudou. **O que saiu da interface:** Previa, impressao em LOTE, `FolhaModeloG3`, `FaixaDePagamento`, `QrDaFaixa`, `lote-de-documentos.ts` e 15 verificacoes — 630 linhas. **O que ficou de proposito e foi conferido no ar:** `GET /faturas/:id/documento` responde **401** (contrato com o CRM, e nenhuma tela nossa o chama mais). **Consequencia aceita pelo dono:** o item 9 do caminho da primeira fatura foi **revogado** — nao ha mais como imprimir o mes inteiro, sao 28 leituras de PDF uma a uma. **14/08/2026 11:47 — A LEVA DE 15/08: O CADASTRO DE FATURA, A SAIDA DA ABA TARIFAS E A REVISAO GERAL.** `origin/main` em `949d085` (8 commits, fast-forward puro — a `main` estava parada em `20b9d02`), **31 migrations = 31**: as **4 pendentes da §7 da `RETOMADA-2026-08-15` foram aplicadas** (`cadastro_da_fatura`, `registro_da_fatura_unificada`, `tarifa_na_unidade_consumidora`, `indices_de_escala_e_login`), as quatro com `applied_steps_count = 1` e sem `rolled_back_at`. Bundle `index-Byobs58d.js` -> **`index-llHfiQQy.js`** (227,5 -> **98,1 KB**, o `lazy` por rota da §5.6), **byte-identico ao compilado no Codespace**, e a aba Documento agora e um pedaco proprio de 64,4 KB. Guarda de arranque: *"client gerado cobre as **37** tabelas de public"* — eram 35, entraram 3 (`modelo_de_fatura`, `campo_personalizado_da_fatura`, `registro_da_fatura_unificada`) e saiu 1 (`tarifa`). **O sinal que prova o codigo novo: `GET /api/tarifas/Equatorial` responde 404** — a rota saiu com a aba —, e nao 401. Catalogo **9/9 contra producao** e dados intactos: **86 clientes, 41 UCs, 0 contratos, 0 faturas**. `migrate deploy` rodou **do Codespace** (o `/etc/financeiro.env` nao tem `DIRECT_URL`) e o classificador **nao barrou** desta vez. Pendencias que continuam sendo do dono e nao de deploy: as **41 UCs estao com `tarifa_reais_por_kwh` NULL** — o conector semeia no proximo `npm run ciclo`, e ate la a composicao do lote **levanta** pela R26, que e o comportamento correto —, o emissor vazio (§7.1) e a `ANTHROPIC_API_KEY` fora do `/etc/financeiro.env`. **14/08/2026 03:41 — A ABA DOCUMENTO E AS DUAS ABAS DA TELA DEFINITIVA.** `origin/main` em `b0c2fff`, **27 migrations = 27** (as duas desta leva ja tinham sido aplicadas de manha), bundle `index-Byobs58d.js` **byte-identico** ao compilado no Codespace, guarda de arranque dizendo *"client gerado cobre as 35 tabelas de public"*. As tres rotas novas respondem **401** de fora (`/faturas/ler-fatura`, `/faturas/ler-boleto`, `/faturas/unificada/compor`) e `/cobranca/layout` responde **404**, que e o que prova que o codigo novo esta rodando — 401 sozinho nao provaria. **O PASSO QUE FALTOU E DERRUBOU O SERVICO POR 30 SEGUNDOS: `npm install` na raiz.** `web:build` roda `npm --prefix web install` e nao instala a raiz; `@anthropic-ai/sdk` entrou nesta leva e o arranque morreu em `ERR_MODULE_NOT_FOUND` cinco vezes seguidas antes de eu ler o log. **O receituario abaixo agora tem esse passo.** Pendente e do dono: `ANTHROPIC_API_KEY` nao esta em `/etc/financeiro.env`, entao as duas rotas de leitura devolvem **503** com a mensagem certa e o resto da aba funciona. **06/08/2026 21:39 — DEPLOY DA SESSAO 23.** `origin/main` em `34d028c`, **25 migrations = 25** (nenhuma nesta leva), bundle `index-DN-7K2b1.js` byte-identico ao compilado no Codespace, guarda de arranque dizendo *"client gerado cobre as 37 tabelas de public"*, catalogo **9/9 contra producao** e dados intactos — **86 clientes, 41 UCs, 0 contratos, 0 faturas, 1 chave Pix**. Corpo anterior abaixo. Mesmo VPS do CRM, **sem alterar uma linha da configuracao dele**. **04/08/2026 02:37 — DEPLOY FEITO.** Producao passou de 21 para **24 migrations** e o bundle e o `index-MNjOhM7U.js`, byte-identico ao compilado no Codespace. `main` avancou **18 commits por fast-forward** (as sessoes 17 a 20; ela estava parada na 16). A guarda de arranque passou dizendo *"client gerado cobre as 36 tabelas de public"* — eram 30. Os **9 invariantes de catalogo passam contra producao**, e o ciclo rodou em seguida preenchendo a coluna nova: **29 `ativado` e 12 `nao_ativado`** (7 em troca de titularidade), com **0 criados, 0 desativados e 0 recusados**. Dados intactos ponta a ponta: **86 clientes, 41 UCs, 0 contratos, 0 faturas** antes e depois |
...
| 8 | **Compor → lançar as tarifas da concessionária → emitir** | 1–7. **06/08: são TRÊS passos e não dois.** `lancarTarifasPorUC` só aceita **rascunho**, e sem ele o total sai só com o crédito — sem erro e sem recusa, porque `valor_total_centavos` é coluna gerada e a parcela ausente vale zero. `Q-TARIFA-CONC-01`, reaberta |
| 9 | ~~**Imprimir o mês inteiro** (aba Documento)~~ **→ voltou a ser uma por vez** | **REVOGADO em 14/08 por decisão do dono**, junto com a remoção do processo anterior da aba Documento (*"quero apenas o que roda na referência"*). A impressão em LOTE existiu de 06/08 a 14/08 e **saiu da interface**: ela imprimia a folha composta da **nossa** fatura, e a folha que ficou sai do **PDF da Equatorial**. Hoje são 28 leituras de PDF, uma por vez. O argumento que criou o lote continua de pé e agora é risco aceito: *"o que falha nisso não é o erro, é a fatura que ninguém percebe que ficou de fora"*. O código está em `git show 66a03f3~1 -- web/src/telas/documento.tsx`, e o bloco no pé daquele arquivo diz o que saiu |
| 10 | **Dono de usina + `regra_repasse`** | **não bloqueia a fatura**, bloqueia o *repasse* |
...
**E o que governa o ADR inteiro é o que a Sicoob suporta.** Lido em 05/08 na documentação pública da Cobrança v3, e **metade da pergunta caiu**: o lado de *saída* ficou sabido — produção, sandbox, OAuth2 em Keycloak, `Authorization: Bearer` mais `client_id`, certificado ICP Brasil por CNPJ —, e sobre **como a Sicoob autentica a chamada ao nosso endpoint não há nada público**. Isso não é *"não tem"*, é *"não é público"*: o pré-requisito deixou de ser ler a documentação e passou a ser **perguntar ao suporte**. Corpo original: HMAC só existe se eles assinarem; mTLS depende da topologia do VPS; cabeçalho depende de a configuração do webhook aceitar um. É a lição da `Q-VIEWSCRED-01` aplicada antes de custar: o ADR escolhe a **forma** que acomoda os três e nomeia a leitura da documentação como pré-requisito, em vez de escolher o mecanismo e descobrir depois.
...
0. **`RETOMADA-2026-08-15.md`** — **se você está retomando o trabalho, leia ESTE primeiro: dois minutos, e diz onde tudo parou.** Substitui a de 14/08. Traz a leva que **fechou a paridade da aba Documento com a referência da Vercel e passou dela**, a funcionalidade nova **Cadastro de Fatura** (emissor com contato, logotipo, chave Pix, modelo/template e campos personalizados), a **saída da aba Tarifas** com a fonte nova entrando antes, o design **remedido** — dois números escritos em comentário estavam errados, e as duas tintas do papel reprovavam AA na fatura que vai ao cliente — e a revisão geral, que achou o `::int` sobre soma de centavos derrubando `GET /carteira` a partir de R$ 21,4 milhões. **A única pendência que sobra é o certificado A1**, que é compra externa. `npm test` EXIT=0, **1.908** verificações · **4 migrations não aplicadas em produção** (§7)
0-a. ~~**`RETOMADA-2026-08-14.md`**~~ — **SUPERADA pela de 15/08.** Corpo datado, mantido pelo registro das duas frentes daquele dia. *(Texto original: substitui a de 08/08. Traz **as duas frentes desta sessão** — a tela definitiva, que substituiu o processo da aba Documento e **está no ar**, e a tarifa, medida e parada em decisão do dono —, os **seis bloqueios com dono nomeado** (três deles de dois minutos), as quatro questões novas, o que **não** foi feito e por quê, e a §7 com as três armadilhas medidas — inclusive o `npm install` que faltava no receituário de deploy e derrubou o serviço por 30 s)
0. ~~**`RETOMADA-2026-08-08.md`**~~ — **SUPERADA pela de 14/08.** Corpo datado, mantido pelo registro da frente da Equatorial. *(Texto original: se você está retomando o trabalho, leia este primeiro.)* Substituía a de 06/08. Traz a fila da primeira fatura (inalterada, sete itens, nenhum é código), a frente nova parada em **duas coisas que também não são código**, as três medições de 07–08/08 que mudaram o que se sabia — inclusive **a data de nascimento não estar no CRM**, conferido em três níveis —, as oito questões com estado, e a §7 com o que **não** fazer em seguida
0. **`PROXIMOS-PASSOS-2026-08-09.md`** — **se a pergunta é "e agora, o que eu faço?", leia depois da `RETOMADA` e pare aqui.** As **duas frentes restantes isoladas** — Equatorial e Sicoob —, cada seção lendo-se sozinha: onde a frente parou, o que fazer que **não é código** com dono nomeado, o que é código e **em que ordem se torna escrevível**, e o que **não** fazer. Não substitui a `RETOMADA` e **não cobre a fila da primeira fatura**, que é a §1 dela. As duas frentes se tocam em **dois pontos e só dois**: o item 1.1 (CPF/CNPJ das 24 pessoas) e a resolvedora do `ADR-0005`
0. ~~**`RETOMADA-2026-08-06.md`**~~ — **SUPERADA pela de 08/08.** Corpo datado, mantido pelo registro do deploy da migration 25 e da conferência da aba Documento. *(Texto original: se você está retomando o trabalho, leia este primeiro: dois minutos, e diz onde tudo parou.)* Substitui o de 03/08. Traz o deploy da migration 25 concluído e conferido, **o portal do Sicoob como próxima ação** — que é do dono e é a coisa mais barata da fila —, a fila sem o que já saiu, o que ainda **não** está publicado e o que **não** fazer em seguida
0. **`PLANO-leitura-fatura-equatorial-2026-08-07.md`** — **a frente nova, e o único documento que revisa a fila inteira contra a nova.** A §0 é a fila vigente remedida e o que esta frente muda nela — o item **1.1 deixou de destravar uma coisa e passa a destravar duas**, porque o login do portal da Equatorial é **UC + CPF**, que é o mesmo insumo. A §2 é o achado que decide o desenho: **a coluna é estreita** — `valor_tarifas_concessionaria` é fio B + iluminação pública + encargos, e o **total a pagar** da fatura é outro número na mesma página, igualmente plausível, cuja troca não gera erro nenhum. A §5 são as **seis questões** que a frente abriu e a §9 é o que eu decidi não fazer
0. **`EQUATORIAL-portal-2026-08-07.md`** — **se você vai abrir o portal da Equatorial, abra esta junto.** Folha de campo: seis passos com lacunas para preencher enquanto navega, e uma tabela de *se vier X, então Y*. O item 6 é o mais importante — **baixar uma fatura real**, sem a qual a `Q-EQTL-CAMPOS-01` não fecha
...
0. **`PENDENCIAS-2026-08-05.md`** — a fila medida em 05/08. **VENCIDA em quatro pontos**, e o topo dela diz quais — a fila vigente é a §3 da `RETOMADA-2026-08-06`. A fila inteira medida contra produção em 05/08, ordenada por *o que destrava o quê* e não por severidade. Diz também o que **não** é pendência: nenhuma migration, e os importadores não precisam de deploy
...
RESUMO-SESSAO-10.md          passagem da sessao 10 — a PAUTA respondida e a
                             carteira inteira
RESUMO-SESSAO-11.md          passagem da sessao 11 — A paleta
...
RESUMO-SESSAO-23.md          passagem da sessao 23 - COMECE POR AQUI. Mitigar o
                             que da, antes do portal da Sicoob, e SEM DECIDIR
                             NADA. A 1 e a aba Documento provada em tres camadas
                             (o dado, o bundle servido byte-identico, e a tela
                             renderizada) - e diz o que a medicao NAO cobre. A 2 e
                             a Q-TARIFA-CONC-01: entre compor e emitir ha um
                             terceiro passo que nenhuma lista tinha, e sem ele a
                             fatura sai menor SEM ERRO - agora e contado, e nao
                             travado. A 3 e o QUARTO caso da Q-PECA-NAO-PLUGADA-01:
                             a retomada dizia "alinhar custa um comando" e o
                             comando NAO EXISTIA - PUT /cobranca/chaves-pix/:id
                             tinha zero chamadores. A 4 sao tres erros meus, e o
                             primeiro e o INSTRUMENTO DE MEDIDA mentindo sobre
                             producao: o harness devolvia zero campos onde
                             producao tem 15, e a foto ia virar prova disso
RESUMO-SESSAO-22.md          passagem da sessao 22 - O documento
                             da fatura passou a sair EM LOTE, e era o ultimo passo
                             do caminho da primeira fatura que ainda era codigo. A
                             2 sao os dois defeitos que so o PDF pega, medidos em
                             Chromium por page.pdf(): imprimir UMA fatura dava
                             CINCO paginas, quatro em branco - e esse era ANTERIOR,
                             o que so se pode afirmar porque a linha de base foi
                             recompilada e medida. A 3 e a lacuna do caminho ponta
                             a ponta: sem `npm run tarifas` entre compor e emitir,
                             a fatura sai cobrando so o credito, sem erro e sem
                             recusa. A 4 mede a Q-FATCHEIA-01 sobre o modelo real:
                             em 2026-06 seriam 0 de 29 cheias, e comissao nenhuma
                             seria gerada na liquidacao
web/src/lote-de-documentos.ts
                             QUEM ENTRA na impressao do mes, quem fica de fora e
                             por que. A invariante e a SOMA: imprimir + fora = o
                             total da competencia, sempre. O padrao NAO e escolha
                             desta tela - e o conjunto que o servidor ja nomeia em
                             `liquidacao.baixar`, citado em vez de decidido, porque
                             `documento.paraFatura` compoe QUALQUER status e
                             descartar em silencio seria a tela inventando regra
RESUMO-SESSAO-21.md          passagem da sessao 21. Os dois
...
RESUMO-SESSAO-20.md          passagem da sessao 20 - Duas
...
RESUMO-SESSAO-18.md          passagem da sessao 18 - As quatro
...
RESUMO-SESSAO-15.md          passagem da sessao 15 - As tres
...
RESUMO-SESSAO-14.md          passagem da sessao 14 — O eixo do
...
# 0. AS DEPENDENCIAS DA RAIZ. `web:build` la embaixo roda `npm --prefix web
#    install` e NAO instala a raiz - o servidor importa de `node_modules/` da
#    raiz. Medido em 14/08: `@anthropic-ai/sdk` entrou numa leva, este passo nao
#    existia, e o arranque morreu em `ERR_MODULE_NOT_FOUND` cinco vezes seguidas.
#    O servico ficou FORA por ~30 s. Barato quando roda, caro quando falta.
sudo -u financeiro env PATH=/opt/financeiro/node/bin:$PATH npm install --omit=dev --no-audit --no-fund
