...
| **No ar** | **`https://financeiro.blackhaus.io`** — systemd, Node 22 isolado, TLS até 26/10. Mesmo VPS do CRM, **sem alterar uma linha da configuração dele**. **O bundle em produção é o de 29/07 (`index-DzZYJ0Ak.js`)** — medido em 30/07 11:36 pelo `index.html` servido: as sessões 14 e 15 **não foram publicadas**. ~~e o `main` ainda não as tem~~ **Corrigido em 30/07 11:36: `main` tem as duas** desde `b1710f5` (30/07 04:19), quando a sessão de UX commitou em cima da de cobrança. O deploy **não exige merge** — é `pull` + `web:build` + `restart`. Só o `RESUMO-SESSAO-15.md` está fora de `main`, e é documento. Comandos e as ressalvas do deploy em `RESUMO-SESSAO-15.md` §7 |
